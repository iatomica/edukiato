...
export const Dashboard: React.FC<DashboardProps> = ({ onViewChange, user }) => {
  const { courses, feed } = useTenantData();
  const nextClass = courses[0];
  const { t } = useLanguage();

  // DEMO: Hardcoded enrollment to demonstrate personalized feed
  const ENROLLED_COURSES: Record<string, string[]> = {
    'sofia@student.com': ['c_jazz_01', 'c_ceramica_01'],
    'liam@student.com': ['c_jazz_01'],
    'maya@student.com': ['c_ceramica_01'],
    'val@student.com': ['c_guitarra_01'],
    'diego@student.com': ['c_guitarra_01'],
  };

  const myCourses = ENROLLED_COURSES[user.email] || [];
  ...
  const StudentStats = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6" id="tour-dash-stats">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="mt-4">
          <h3 className="text-3xl font-bold text-slate-800">95%</h3>
