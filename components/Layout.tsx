...
import { Home, BookOpen, Users, Calendar, Settings, LogOut, Menu, Bell, MessageSquare, DollarSign, BarChart2, Globe, Info, Building2, ChevronDown } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      `}>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          {can('read', 'student') && can('create', 'course') && (
            <NavItem view="students" icon={Users} label={t.nav.students} />
          )}
          ...
          {can('read', 'financial') && (
            <>
              ...
              <NavItem view="financials" icon={DollarSign} label={t.nav.financials} />
              <NavItem view="reports" icon={BarChart2} label={t.nav.reports} />
            </>
          )}
        </nav>
              ...
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-scale-in origin-top-right">
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.map(notif => (
                    <div key={notif.id} className={`p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors ${!notif.isRead ? 'bg-primary-50/30' : ''}`}>
